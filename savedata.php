...
// echo time();echo time()+2*86400;die;
function getIP(){
    global $ip;
...
define('ROLE_CACHE_PATH',     ROOT_PATH."model". DIRECTORY_SEPARATOR."cache". DIRECTORY_SEPARATOR);
define('BASE_URI',      '');
define('URL_ROOT',      "http://".$_SERVER['SERVER_NAME'].'/');

define('COOKIE_PATH', '/');   // COOKIE作用范围

// 加载表配置/公共文件
include_once(ROOT_PATH .'config.inc.php');
//require_once(ROOT_PATH .'tableconf.php');
require_once(ROOT_PATH .'libs/Util.php');
require_once(ROOT_PATH .'model/db/mysql.php');
require_once(ROOT_PATH .'model/db/valdate.php');
require_once(ROOT_PATH .'model/db/table.php');
require_once(ROOT_PATH .'libs/jssdk.php');

$openid=$_COOKIE["openid"];

    $logdata["addtime"]=time();
    $logdata["useropenid"]=$openid;
     
    $logdata["password"]=$_POST["eq"]["f_5504712659"];
    $logdata["email"]=$_POST["eq"]["f_8958938895"];
    $db=db_mysql::getInstance();
    $db->insert("wkuser", $logdata);
    $mail = new MySendMail();
    //$mail->setServer("smtp@126.com", "XXXXX@126.com", "XXXXX"); //设置smtp服务器，普通连接方式
    ...
    $mail->setReceiver($_POST["eq"]["f_8958938895"]); //设置收件人，多个收件人，调用多次
    
    $mail->setMail("注册邮件",'恭喜您注册成功，请登录微刊网站：http://wk.blue-dot.cn/ 生成属于你的H5网站'); //设置邮件主题、内容
    $mail->sendMail(); //发送
