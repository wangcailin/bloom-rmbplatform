...
$db=new MyDB($config->db);
$todayzero=strtotime(date("Y-m-d"));;
$yestodayzero=$todayzero-86400;
$res=$db->loadToArray("select * from yiyingli where addtime<$todayzero and addtime>=$yestodayzero");
$str= "<table border=1 >";
$str.= "<tr><td>No.</td><td>OPEN ID</td><td>First Name</td><td>Last Name</td><td>Company Name</td><td>E-mail</td><td>Position</td><td>DC code</td><td>Cell</td><td>Address</td><td>Area</td><td>Post Code</td><td>number of Teams</td><td>Time-stamp</td><td>Time</td></tr>";
foreach ($res as $row)
{
    $str.= "<tr><td>".implode("</td><td>",$row)."</td><td>".date("Y-m-d H:i:s")."</td></tr>";
}

 $mail = new MySendMail();
//$mail->setServer("smtp@126.com", "XXXXX@126.com", "XXXXX"); //设置smtp服务器，普通连接方式
$mail->setServer("smtp.exmail.qq.com", "yu.cao@blue-dot.cn", "bluedot@2015", 465, true); //设置smtp服务器，到服务器的SSL连接
$mail->setFrom("yu.cao@blue-dot.cn"); //设置发件人
$mail->setReceiver("info@sh.squaremilerelay.com"); //设置收件人，多个收件人，调用多次
$mail->setReceiver("kate@ggp.hk");
$mail->setReceiver("stone@ggp.hk");
$mail->setReceiver("wing@ggp.hk");
$mail->setReceiver("hannah.han@blue-dot.cn");
$mail->setMail(date("Y年m月d日",$yestodayzero)."数据",$str); //设置邮件主题、内容
 $mail->sendMail(); //发送
